...
- **Deletion.** Tasks use soft delete (`deleted_at`); areas/goals/projects have NO `deleted_at` — hierarchy deletes are hard (areas/goals via `AreaRepository`/`GoalRepository`, projects via `ProjectRepository`); the store removes an area's goals explicitly.  every repository read filters it. Work sessions and habit logs hard-delete only via explicit calls (`deleteSession` for cancellation; habit logs are never deleted, only overwritten). The schema's `ON DELETE SET NULL` / `CASCADE` rules mean removing a parent never destroys historical child records.
...
Phase 2C additions: `goal.parked` ({goal_id, parked_until}), `goal.acknowledged` ({goal_id}), `goal.card_added` ({goal_id, task_id} — written only after the task row exists). Migration 005 adds `goals.parked_until TEXT` (the local day a parked goal was parked; `goals.status` already carried 'paused' in its 001 CHECK). Planner-fixes addition: `task.deleted` (`{title}`; soft delete via `deleted_at`, refused while the task owns a live session). Phase 2B additions: `task.completed` / `task.deferred` (dedicated lifecycle events emitted alongside the generic `task.status_changed`, with `{estimated_minutes, scheduled_date}`), `compression.applied` (`{date, deferred_count, deferred_minutes}`), and `planning.day_snapshot` — the plan-of-record for a day (`{date, available_minutes, primary_objective, total_planned_minutes, planned_tasks[], snapshot_reason}`; reasons: day_opened / compression_applied / material_replan; deduped by content signature; written through a serialized queue so a StrictMode double-boot cannot duplicate). `session.cancelled` now logs **before** row deletion and carries `duration_seconds` (the row itself stays deleted).

The event log is also a read model since Phase 2B: `getByDateRange` (indexed, optional type filter), `getByType`, `getLatestSnapshotsForRange`. Payloads have typed Zod schemas (`src/domain/events/payloads.ts`) parsed defensively — malformed historical events become coverage warnings, never crashes. Full semantics: [SEMANTICS.md §9](SEMANTICS.md).
