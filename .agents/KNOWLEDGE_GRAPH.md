...
| Phase 2C (classification/balance/orphans) | ✅ VERIFIED | native via CDP: migration v4→v5 + integrity ok; execution-balance readout renders from live events (suppressed on sparse weeks); orphan goal created+aged → section renders; park writes status+parked_until + goal.parked; ack logs goal.acknowledged + excludes on refetch; Add-a-card creates the task with project_id + today and logs goal.card_added; all writes survive force-kill + relaunch (WAL); zero console errors. 229/229 tests (28 suites) | 2026-09-14 |
