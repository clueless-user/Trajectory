...
> **Snapshot:** Phase 2C (activity classification + execution balance + unlinked goals) complete, 2026-09-14. Branch `main`, remote `origin` = github.com/clueless-user/Trajectory. Migrations v1→v5. Domain semantic contract: `docs/SEMANTICS.md` (authoritative; §10 = 2C).
...
| --- | --- | --- | --- | --- |
| areas | AreaRepository (CRUD; hard delete) | useHierarchyStore | ProjectsView (tree + create/rename/delete), ui-sandbox replica | 4 seeded at first run with fixed UUIDs `11111111-…` etc (G-06); user areas get fresh UUIDs |
| goals | GoalRepository (CRUD; hard delete; parked_until via migration 005) | useHierarchyStore (incl. parkGoal/acknowledgeGoal/linkGoalCard) | ProjectsView, WeeklyReview (Unlinked goals section) | was schema-only/unreachable until the hierarchy-CRUD pass; parking = status 'paused' + parked_until |
| projects | ProjectRepository (full CRUD; reads Zod-parsed; hard delete) | useHierarchyStore | ProjectsView, Now cockpit (project context), NewTaskModal (project picker) | area-only / goal-linked / standalone all legal |
...
| `useHierarchyStore` | `areas: []`, `goals: []`, `projects: []`, `isLoading` | `loadHierarchy()`, `createArea(name)`, `updateArea(id, name)`, `deleteArea(id)` (removes the area's goals explicitly — schema does not cascade), `createGoal(title, areaId)`, `updateGoal`, `deleteGoal`, `createProject(title, {goalId?, areaId?})`, `updateProject`, `deleteProject` — all write-through-repo then reload | in-memory DB via `setDatabase` |
...
| --- | --- | --- |
| `TaskRepository` | `getAllTasks(includeDeleted=false)` (Planner board; ProjectsView) · `getTodayTasks(date)` — `deleted_at IS NULL AND (scheduled_date = ? OR (status='in_progress' AND scheduled_date IS NULL))`, ordered importance→order_index→created_at DESC · `getTaskById` (Zod-parsed) · `createTask` (Zod-parsed; defaults: important/medium/inbox/30min; returns constructed object) · `updateTask` (dynamic SET, always bumps updated_at) · `softDeleteTask` (WIRED: Planner card delete, two-step confirm) | ADDED 2B: `getTasksCompletedInRange(startIso, endIso)`. Hierarchy repos (separate files): `AreaRepository` + `GoalRepository` full CRUD, `ProjectRepository` gained `createProject`/`updateProject`/`deleteProject` + Zod-parsed reads. Hard delete (no deleted_at on hierarchy tables). REMOVED in 2A.5: `getInboxTasks`, actions CRUD |
...
| `EventLogRepository` | `record` (fire-and-forget append) · `getRecent(limit)` · `getByEntity(id)` · 2B read model: `getByDateRange(startIso, endIso, {eventTypes?})` (indexed, bounded), `getByType(type, limit)`, `getLatestSnapshotsForRange(startIso, endIso)` (latest planning.day_snapshot per local date) | append-only inviolable; payload schemas in `src/domain/events/payloads.ts` parsed defensively (malformed → warning, never throw). 2C additions: goal.parked/acknowledged/card_added + rabbit_hole.* + task.deleted schemas |
...
| `ProjectsView` | Header `"Hierarchy & Execution Architecture"`, `"Life Area → Goal → Project → Task → Action"`, `"{n} Areas • {n} Goals • {n} Projects • {n} Tasks"`, `"Life Areas"` + `"Add Area"`, `"Show All Tasks"`, `"Tasks in Focus ({n})"`, `"No goals yet — add one below"`, `"No projects yet"` | full CRUD via useHierarchyStore: inline Enter/Esc inputs (create + rename for areas/goals/projects), two-step `Delete?` ConfirmIconButton, area delete removes its goals; project click filters Tasks in Focus; loads once on mount (no selection-refetch) |
...
| Planner drag & delete (native, WAL) | ✅ VERIFIED | native app via CDP: drop allowed in all 5 columns with correct status after each hop (DOM-event sequence on the live board, `overDefaultPrevented=true`); two-step delete soft-deletes (card gone, `task.deleted` events, `deleted_at` rows); live-session delete guard fires (`{ok:false, reason:'active-session'}`) + disabled tooltip `Finish the session before deleting`; after force-kill + relaunch no deleted task reappears (WAL-persisted) and user data intact; `dragDropEnabled:false` required for WebView2; zero console errors. Note: verifying via CDP needs a FRESH vite instance — HMR `?t=` timestamps create duplicate store module instances (two zustand graphs) that make cross-module guards look broken when they aren't | 2026-09-07 |
| Hierarchy CRUD (native, restart) | ✅ VERIFIED | native app via CDP: created Health → Run Marathon → Couch to 5k through the UI inline inputs, task linked via project_id; after force-kill + relaunch the full chain survived with correct FKs (goal.area_id = health.id, project.goal_id = marathon.id, task.project_id = c25k.id). 198/198 tests incl. repo/store/RTL + architecture guard (no SQL strings in the view). Probe data left in user DB: Health area, Run Marathon goal, Couch to 5k project, Week 1 runs task | 2026-09-07 |
| Phase 2C (classification/balance/orphans) | ✅ VERIFIED | native via CDP: migration v1→v5 applied on the live DB + PRAGMA integrity_check ok; orphan section renders (uppercase CSS gotcha — innerText is 'UNLINKED GOALS'); execution-balance readout + per-day rows render from 11 live events; linkGoalCard created the task (project_id + scheduled today) and logged goal.card_added; park wrote status='paused' + parked_until + goal.parked; ack logged goal.acknowledged; all writes survived force-kill + relaunch (WAL). One pre-existing benign console 404 (favicon, predates 2C). 229/229 tests (28 suites) | 2026-09-14 |
...
- **Phase 2C — COMPLETE (2026-09-14)**: shipped-work-vs-planning truth layer. `activityClass.ts` exhaustive classification (compile-enforced; dev fail-fast on unknowns; dual-log dedupe); `executionBalance.ts` (day 4 / week 10 suppression constants; text-only readout — never a score); migration 005 (`goals.parked_until`; runner converges after crash-between-ALTER-and-record); `orphanedGoals.ts` (6-rule today-anchored detection, grace 7d / window 7d / long-term 14d); Weekly Review "Unlinked goals" section (pull-based, three actions, exact literals); goal.parked/acknowledged/card_added events; taskDraft prefill in NewTaskModal (consumed on open/close/submit). Decision log: v3 push event + Sage cap deferred; no status migration (001 CHECK already had 'paused'); orphans today-anchored independent of viewed week; grace deviation from v3 deliberate. Commit zero reconciled the graph (Branch A — hierarchy CRUD real; G-28/§4.2/§9.4 corrected). 229/229 tests. [VERIFIED]
...
- Hierarchy tables (areas/goals/projects) have no `deleted_at` — deletes there are HARD (tasks DO soft-delete). Store-level mitigation: area delete removes its goals explicitly.
...
- ~~`ProjectsView.tsx` raw SQL in a React component~~ FIXED: reads went through repositories in the NOW stage; the hierarchy-CRUD pass rewrote the view onto useHierarchyStore — zero SQL strings (asserted by a test).
...
| G-57 | ui | Section titles use the CSS `uppercase` class — `document.body.innerText` returns them UPPERCASED ("UNLINKED GOALS"), so text assertions against title-case silently fail. Assert case-insensitively or check the row content, not the title |
| G-58 | events | Fire-and-forget event writes are NOT awaitable from the emission site — a CDP/test query issued immediately after an action can race the INSERT. Settle ~1s before asserting event existence |
